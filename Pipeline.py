...
def remove_horizontal(x, y, i, img, mode="open"):
    ...
    remove_horizontal = cv2.morphologyEx(
        img, mode, horizontal_kernel, iterations=i)
...
def remove_vertical(x, y, i, img, mode="open"):
    ...
    remove_vertical = cv2.rotate(
        remove_vertical, cv2.ROTATE_90_COUNTERCLOCKWISE)
...
def find_text(orig, psm=13, oem=3, digits=False):
    ...
    contours, hierarchy = cv2.findContours(
        invert, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    ...
    num_rows, num_cols, tokens, tokenCoords, tableCoords = AClustering(
        orig.copy(), finalContours, psm, oem, digits)
...
def hconcat_resize_max(im_list, interpolation=cv2.INTER_CUBIC):
    ...
    for im in im_list:
        ...
        im2 = cv2.copyMakeBorder(
            im, top, bottom, 10, 10, cv2.BORDER_CONSTANT, None, value=[255, 255, 255])
        new_list.append(im2)
...
def convert(img, psm=6, oem=3, digits=False):
    ...
    ext = pytesseract.image_to_string(
        gray, config=config + " -c preserve_interword_spaces=1 -c digit_or_numeric_punct=0", lang='eng')
